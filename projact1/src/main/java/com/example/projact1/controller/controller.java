...
import com.example.projact1.dto.CalcDTO;
...
public class controller {
    ...
    // Sting : redirect 하는 경우이거나 template 파일명을 임의대로 지정하는 경우

    // 입력값 가져오기
    // 1) HttpServletRequest 사용가능(입력값을 가져오는 용도로 잘 사용하지 않음)
    // 2) 매개변수 선언(변수명과 이름을 맞추는게 변함)
    // 3) DTO 사용(post 메소드가 끝난 후 보여지는 페이지에서 DTO 사용가능)
    // CalaDTO => calaDTO / LoginDTO =>loginDTO 선언한 변수의 이름 아님 변수명 앞자리 소문자변경

    // 컨트롤러가 가지고 있는 값을 화면과 공유하기
    // redirect 움직이지 않는 경우
    // 1) ~~DTO : 기본 공유 됨(클래스명과 동일(맨 첫 자만 소문자로))
    // 2) 변수에 들어있는 값을 공유 : model.addAttribute("이름",변수명)
    // model.addAttribute("이름",객체명)
    // 3) method(@ModelAttributes int bno) : bno 공유하고 싶으면
    // 4) method(@ModelAttributes("u") UsetDTO uDto) : UserDTO 공유하지만 이름을 다르게 공유

    // redirect 움직이는 경우
    // RedirectAttributes rttr 이용
    // 1) rttr.addAttribute("이름",값) : 경로에 ? 다음에 따라가는 값의 형태 => ${param.이름}
    // 1) rttr.addFlashAttribute("이름",값) : 세션을 이용하기 때문에 따라가는 건 안 보인 =>${이름}

    // Model 과 RedirectAttributes 차이점 : 움직이는 방식 / 객체을 담을 수 있나? 없나?
    ...
    public String basic2(RedirectAttributes rttr) {
        ...
        rttr.addFlashAttribute("addr", "서울");

        // sendRedirect(); : redirect:경로
        return "redirect:/ex1"; // http://localhost:8080/ex1?age=15
        // return "redirect:/ex1?age=15"; //http://localhost:8080/ex1?age=15&name=hong
    }
